...
*emmeans* del inglés "Estimated Marginal Means" proporciona métodos para obtener medias marginales estimadas (EMMs, también conocidas como medias de mínimos cuadrados) esta librería perminte calcular contrastes o combinaciones lineales de dichas medias marginales con varios ajustes por multiplicidad. También se pueden estimar y contrastar pendientes de líneas de tendencia.
...
**Ejemplos de ANOVA de dos vías (two-way ANOVA)**   
...
boxplot(Count ~ Treat, data = Rev8.16_fac)
boxplot(Count ~ Sens, data = Rev8.16_fac)

boxplot(Count ~ Sens + Treat, data = Rev8.16_fac)

ggplot(data = Rev8.16_fac, aes(x = Sens, y = Count) )  + 
  geom_boxplot() + 
  geom_jitter(width = 0.1, alpha=0.6, aes(color = Treat))
