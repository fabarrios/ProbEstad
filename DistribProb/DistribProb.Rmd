...
$$ f(x) =  _nC_x p^x (1-p)^{n-x} \hspace {20pt} para \; x = 0, 1, 2, \dots , n $$
$$ f(x) = 0, \hspace {20pt} de\; otra\; forma.$$
Esta expresión es la **Distribución Binomial**, en general os valores de $p(x)$ para una distribución Binomial, se pueden estimar a partir de tablas de valores de $p$, pero en R se calculan con la familia de funciones `dbinom, pbinom, qbinom y rbinom`.  
