...
## Otro ejemplo  
Del ejercicio 4.2.1 del Daniel: Del estudio de Cross, a los pacientes que estaban en el programa de tratamiento para adiciones al juego. Se les preguntó si declaraban otra adicción a alcohol o substancias.  Si la variable X de la tabla 4.2.4 presenta la distribución de frecuencias de la variable aleatoria discreta X relacionada con el número de otras addiciones además del juego que declaran los participantes de este estudio.  Construya una tabla de frecuencias relativas y de frecuencias acumuladas para la variable discreta del número de adiciones.  Construya una gráfica de la distribución de las probabilidades.
[Table 4.2.4](https://fabarrios.github.io/ProbEstad/Notas/StudyCrossTable_ch4)

# Distribución Binomial  
Cuando un ensayo (toma de dato) aleatorio solo puede suceder de una de dos formas posibles, mutuamente excluyentes (vivo-muerto, enfermo-sano, mujer-hombre) se llama un ensayo de Bernoulli.  
...
Los parámetros de una distribución binomial son dos: *n* y *p* (*n* el número de ensayos y *p* la probabilidad de éxito del proceso), son suficientes para estimar la distribución binomial. Y la expresión para el promedio y varianza de una distribución binomial están dados por: $\mu = np$ y $\sigma^2 = np(1-p) = npq$.  
...
2. En teoría pueden suceder un número infinito de ocurrencias en el intervalo.  
...
En R la distribución de Poisson se estima por los comandos `dpois, ppois, qpois, y rpois`

**Ejemplo** En un estudio de anafilaxis entre pacientes que fueron expuestos a anestesia con rocuronio bromuro, laake y Rottingen encontraron que los casos de anafilaxis siguen una distribución de Poisson con $\lambda = 12$ incidentes por año en Noruega. Encuentre la probabilidad de que en el año siguiente a este estudio, en pacientes que fueron anestesiados por rocuronio bromuro, se pudieran dar exactamente tres casos de anafilaxis.  
*respuesta* La respuesta es $P(X=3)$ en una distribución de poisson. estimando esta probabilidad usando R es: dpois(3, 12)  
```{r}
dpois(3, 12)
```

**Ejemplo** del mismo ejemplo anterior cual es la probabilidad de que por lo menos tres pacientes en el año siguiente al estudio experimenten anafilaxis si rocuronio bromuro es usado para anestesiarlos.  
...
**Ejemplo** En un estudio de un organismo acuático, una gran cantidad de muestras del mismo volumen son tomadas de un estanque de agua, y el número de organismos del estudio son estimados.  El promedio de organismos fue determinado en dos por muestra.  Suponiendo que el número de organismos sigue una distribución de Poisson, encuentre la probabilidad de que la siguiente muestra contenga uno o menos organismos?.  
...
**Ejemplo** cual es la probabilidad de un número aleatorio *z* tomado al azar, esté entre $-2.55$ y $2.55$?    
