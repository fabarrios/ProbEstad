...
**Definición** Un estimado de intervalo consiste en dos valores numéricos que definen el rango de valores entre los que, con algún grado de confianza, muy posiblemente se incluye el parámetro de la población que se está estimando.  
...
```{r}
maxi <- 84.3 + qnorm(.005, lower.tail = FALSE)*sqrt(144/15)
mini <- 84.3 + qnorm(.005, lower.tail = TRUE)*sqrt(144/15)

print(c("Intervalo: ", mini, maxi), digits = 4, quote = FALSE)
```
...
A principio del siglo veinte se publicaron varios trabajos de suma importancia por un estadístico de nombre "Student". En realidad Student era en nombre de autor de William Gosset.  
William Sealy Gosset (1876-1937), el estadístico inglés que desarrolló la distribución t y la prueba t mientras trabajaba como cervecero en la cervecería Guinness en Dublín, se vio obligado a publicar bajo el seudónimo “Student” debido a las estrictas políticas de confidencialidad de su empleador. Esta política de secreto, desencadenada por un incidente anterior en el que un investigador de Guinness reveló secretos comerciales en una publicación, dio forma a la recepción del trabajo de Gosset. El seudónimo ocultó su identidad durante décadas, retrasó el reconocimiento de sus contribuciones y creó una curiosa dinámica entre Gosset, Karl Pearson y Ronald Fisher que influyó en el desarrollo de la estadística moderna.  
William Gosset fue contratado por Guinness como químico y cervecero y rápidamente se convirtió en jefe cervecero experimental; más tarde fue ascendido a jefe cervecero de la cervecería Guinness Park Royal y, en última instancia, a jefe cervecero para todo Guinness.  
La tarea de Gosset como aprendiz de cervecero era evaluar la calidad de las materias primas (cebada malteada, lúpulo y levadura) de una manera rentable. Esto requirió sacar conclusiones de un pequeño número de observaciones experimentales, ya que las pruebas a gran escala eran poco prácticas y costosas. Fue precisamente esta restricción práctica la que lo llevó a desarrollar lo que se convirtió en la distribución t de Student.  


## La distribución t  

Hasta ahora hemos establecido los procedimientos para estimar el intervalo de confianza de un proceso de muestreo, y en dicho proceso se ha dado por conocida la varianza de la población de donde se extrajeron las muestras.  Por lo general si no se tiene el promedio de la población es muy posible que tampoco se conozca a varianza, y por lo tanto se tendrá que estimar de la muestra. lo que genera un problema. Entre las herramientas con las que contamos para resolver este problema se sabe que la estadística  

$$ z=\frac{\bar{x}-\mu}{\sigma/\sqrt{n}} $$
está distribuida normalmente cuando la población está distribuida normalmente o por lo menos está distribuida de forma parcialmente normal para valores grandes de $n$, independientemente de la distribución de la población. Se puede usar la desviación estándar de la muestra  
...
3. Por lo general tiene una varianza mayor que $1$, pero la varianza se aproxima a $1$ cuando el tamaño de la muestra se hace grande. Y por lo general para $df (degrees of fredom)$ grados de libertad, para $df >2$ la varianza de *t* es $df/(df -2)$. Como $df = n-1$ para $n>3$ la varianza de la $t$ se puede escribir como $(n-1)/(n-3)$ para $n>3$.  
...
6. Comparado con la distribución normal la $t$ tiene un máximo central más bajo y cuerpo central más angosto y las colas más anchas.  
7. La distribución $t$ tiende a la distribución normal para $(n-1) \rightarrow \infty$.  
...
*Para varianzas iguales* Si se puede suponer que la varianza de las poblaciones es igual y se quiere estimar la diferencia del promedio de las poblaciones se puede usar lo que se conoce como un *valor combinado* de las varinazas muestrales (pooled estimate).  
...
El número de grados de libertad para estimar el valor de *t* para usar en la estimación del intervalo esta dado por $df = n_1 + n_2 -2$.  
...
Para estimar un intervalo de confianza de la varianza poblacional, se puede usar esta distribución, y un intervalo $100(1-\alpha)$ por ciento de la varianza se puede estimar a partir de  
...
Con estos datos queremos estimar la varianza de IAA en unidades de IAA de la población de donde se tomó esta muestra y construir el intervalo de confianza al $95$ por ciento $(\alpha = .05)$ de dicha estimación.  
...
CI_sig <- function(x,alpha) {
  ...
  print(c("Desviación estandar estimada", sd_x), quote = FALSE)
  print(c("Intervalo de confianza de la desviación estándar"), quote = FALSE)
  print(c(lbs, las))
}
...
**Ejemplo** Allen y Gross examinaron la fuerza del flexor del dedo gordo del pie de pacientes con fascitis plantar, una condición común en pacientes con problemas musculoesqueléticos. El tratamiento de la facitis plantar es un tratamiento costoso y muchas veces poco efectivo.  Una de las medidas de base de los paciente fue su índice de masa corporal (BMI, por sus siglas en inglés). Para 16 de las mujeres de este estudio, la desviación estándar para BMI fue de $s_1 = 8.1$ y para los cuatro hombres en el estudio, la desviación estándar fue de $s_2 = 5.9$. Queremos considerar un intervalo de confianza al 95% para la proporción de las desviaciones estándar de las poblaciones en el estudio.  
...
Para obtener los valores críticos de $F_{(1-\alpha/2), df_1, df_2}$ o $F_{\alpha/2, df_1, df_2}$ se usó la funcion base de R que estima el área bajo la curva de la distribución *F*: `qf(.975, df1, df2)` o `qf(.025, df1, df2)`.  

**Ejemplo** El propósito del estudio de Moneim et al. fue examinar amputaciones del dedo índice por accidentes durante competencias de laso en rodeos. Los investigadores revisaron 16 casos de amputaciones de índice. De estás, 11 fueron amputaciones totales mientras 5 fueron parciales. El tiempo de isquemia es la longitud de tiempo de suministro de oxígeno insuficiente al índice amputado.  

Los tiempos de isquemia (en horas) para los sujetos que experimentaron amputación completa fueron:  
...
*Respuesta* Usando la teoría presentada anteriormente puedo desarrollar la función `PropVar_CI` para calcular el intervalo de confianza para la proporción de las varianzas de los dos grupos de datos, descrita a continuación:  
...
Y de forma mucho más directa se puede usar la función `var.test`, incluida en la librería base de R, que nos da la misma repuesta de forma directa, la cual además de estimar el intervalo de confianza nos da una prueba completa con pobabilidad de la poporción de las varianzas:  
