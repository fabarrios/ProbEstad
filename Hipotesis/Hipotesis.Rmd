...
**Pasos para la Prueba de Hipótesis**, por motivos de conveniencia la prueba de hipótesis será presentada en un procedimieeto de diez pasos.  
...
Infants <- read_csv("DataSets/ch07_all/EXR_C07_S03_10.csv")
...
*Respuesta* Si la cirugía incrementa la función biliar, esto es incrementa GBEF, $\mu_{POSTOP} > \mu_{PREOP}$, es decir queremos demostrar que la diferencia en GBEL es positiva. Por lo tanto, la hipótesis nula es que $H_0: \mu_{POSTOP - PREOP} \leq 0$ y por lo tanto $H_A: \mu_{POSTOP - PREOP} > 0$

```{r}
GBEF <- read_csv("DataSets/ch07_all/EXA_C07_S04_01.csv")
boxplot(GBEF$POSTOP, GBEF$PREOP, xlab = "POSTOP, PREOP", ylab = "Función biliar")
t.test(GBEF$POSTOP, GBEF$PREOP, alternative = "greater", mu=0, paired = TRUE, var.equal = TRUE, conf.level = 0.95)
```
...
```{r}
GDS <- read_csv("DataSets/ch07_all/EXr_C07_S04_01.csv")
